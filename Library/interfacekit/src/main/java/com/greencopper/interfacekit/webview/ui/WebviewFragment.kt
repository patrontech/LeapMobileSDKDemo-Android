package com.greencopper.interfacekit.webview.ui

import com.greencopper.core.data.KiboSerializable
import com.greencopper.core.metrics.Screen
import com.greencopper.core.metrics.events.ScreenViewEvent
import com.greencopper.core.services.track
import com.greencopper.interfacekit.metrics.webview
import com.greencopper.interfacekit.navigation.layout.RedirectableLayout
import com.greencopper.interfacekit.navigation.layout.RedirectionHash
import com.greencopper.interfacekit.webview.WebViewLayoutData
import com.greencopper.toolkit.App

internal class WebViewFragment : BaseWebViewFragment<WebViewLayoutData>, RedirectableLayout {

    constructor(webViewData: WebViewLayoutData) : super(webViewData)

    @Deprecated("To conform to system and provide to FragmentFactory an empty constructor accessed by reflection")
    constructor() : super(null)

    override val redirectionHash: RedirectionHash
        get() = data.redirectionHash

    override fun onResume() {
        super.onResume()
        App.track(ScreenViewEvent(Screen.webview(data.analytics.screenName)))
    }

    override fun restoreData(encodedData: String): WebViewLayoutData = KiboSerializable.decodeFromString(encodedData)
}
