...
public class UnregisteredAccountWidgetInitializerTest {
    private lateinit var initializer: UnregisteredAccountWidgetInitializer

    @BeforeEach
    public fun beforeEach() {
        Toolkit.setupTest()

        initializer = UnregisteredAccountWidgetInitializer()
    }
    ...
    public fun resolveLayout_shouldGetLayout() {
        ...
        //then
        assertThat(result).isInstanceOf(UnregisteredAccountWidgetLayout::class.java)
    }

    @Test
    public fun resolveParams_withCorrectParams_shouldGetDeserializedParams() {
        //given
        val data = UnregisteredAccountWidgetParameters(
            description = "description",
            button = UnregisteredAccountWidgetParameters.Button(
                title = "New widget",
                onTap = UnregisteredAccountWidgetParameters.OnTap(
                    routeLink = "routeLink",
                    analytics = ItemNameAnalytics(
                        itemName = "unregistered_account_widget"
                    )
                )
            )
        )
        ...
        //then
        assertThat(result).isInstanceOf(UnregisteredAccountWidgetParameters::class.java)
