...
@Disabled("Causing error randomly: Failed to find ColorBuffer")
@OptIn(ExperimentalTestApi::class)
internal class PressableColorComposableTest {
    ...
    private lateinit var pressableColor: TestPressableColor
    ...
    internal fun setUp() {
        ...
        pressableColor = TestPressableColor(InterfaceKitColor)
    }

    @Test
    fun accessColors_shouldReturnDefaultColors() {
        extension.use {
            setContent {
                ...
                CompositionLocalProvider(
                    mockColors(
                        mapOf(
                            ...
                            listOf("interfaceKit", "test", "pressed") to ColorCompose.Blue,
                        )
                    )
                ) {
                    pressableColor.normal shouldBe ColorCompose.Red
                    pressableColor.pressed shouldBe ColorCompose.Blue
                }
            }
        }
    }
        ...
        extension.use {
            setContent {
                ...
                CompositionLocalProvider(
                    ...
                    pressableColor.pressed shouldBe ColorCompose(ColorAndroid.BLUE)
                }
            }
        }
    }

    class TestPressableColor(parent: UIColor) : PressableColorComposable(parent) {
        override val level: String = "test"
        override val normalDefault: () -> Color = { Color(ColorAndroid.RED, null) }
        override val pressedDefault: () -> Color = { Color(ColorAndroid.BLUE, null) }
    }
}
