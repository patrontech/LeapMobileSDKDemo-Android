...
public class AccountProfileWidgetInitializerTest {
    private lateinit var initializer: AccountProfileWidgetInitializer

    @BeforeEach
    public fun beforeEach() {
        Toolkit.setupTest()

        initializer = AccountProfileWidgetInitializer()
    }
    ...
    public fun resolveLayout_shouldGetLayout() {
        ...
        //then
        assertThat(result).isInstanceOf(AccountProfileWidgetLayout::class.java)
    }

    @Test
    public fun resolveParams_withCorrectParams_shouldGetDeserializedParams() {
        //given
        val data = AccountProfileWidgetParameters(
            AccountProfileWidgetParameters.Provider(AccountProvider.Key("accountProvider", 1)),
            listOf(AccountProfileWidgetParameters.Info("email", "key"))
        )
        ...
        //then
        assertThat(result).isInstanceOf(AccountProfileWidgetParameters::class.java)
    ...
    public fun serialize_deserialize_test() {
        testKiboSerializable(
            AccountProfileWidgetParameters(
                AccountProfileWidgetParameters.Provider(AccountProvider.Key("accountProvider", 1)),
                listOf(AccountProfileWidgetParameters.Info("email", "key"))
            )
        )
