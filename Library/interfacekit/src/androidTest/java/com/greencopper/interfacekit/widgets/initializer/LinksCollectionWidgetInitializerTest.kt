...
internal class LinksCollectionWidgetInitializerTest {

    private lateinit var initializer: LinksCollectionWidgetInitializer

    @BeforeEach
    fun beforeEach() {
        Toolkit.setupTest()

        initializer = LinksCollectionWidgetInitializer()
    }
    ...
    fun resolveLayout_shouldGetLayout() {
        ...
        //then
        assertThat(result).isInstanceOf(LinksCollectionWidgetLayout::class.java)
    }

    @Test
    fun resolveParams_withCorrectParams_shouldGetDeserializedParams() {
        //given
        val data = LinksCollectionWidgetParameters(
            "title",
            listOf(
                LinksCollectionWidgetParameters.Link(
                    LinksCollectionWidgetParameters.Link.Icon(true, "color", "color"),
                    "text",
                    "a11yLabel",
                    "routeLink",
        ...
        //then
        assertThat(result).isInstanceOf(LinksCollectionWidgetParameters::class.java)
    ...
    fun serialize_deserialize_test() {
        testKiboSerializable(
            LinksCollectionWidgetParameters(
                "title",
                listOf(
                    LinksCollectionWidgetParameters.Link(
                        LinksCollectionWidgetParameters.Link.Icon(true, "color", "color"),
