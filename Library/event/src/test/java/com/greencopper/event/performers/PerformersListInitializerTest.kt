...
internal class PerformersListInitializerTest {
    private val initializer = PerformersListInitializer()
    ...
    fun whenGettingLayout_withCorrectParams_shouldGetLayout() {
        ...
        val widgetInfo: WidgetCollectionConfiguration.Instance.WidgetInfo =
            KiboSerializable.decodeFromString(
                """
                            {
                              "key": {
        ...
        val parameters = PerformersListData(
            TopBarData(),
            "kibo://performerDetail",
            null,
            null,
            true,
            listOf(
    ...
    @Test
    @DisplayName("Given a null PerformersListData.Search object, When build is called, Then a null object is returned")
    fun buildSearchShouldReturnNull() {
        val result = Search.build(null)
