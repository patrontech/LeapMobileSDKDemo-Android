...
internal class MyActivitiesManagerTest : CoroutineTest(UnconfinedTestDispatcher()) {
    ...
    private val classUnderTest = MyActivitiesManager(
        lazyLocalStorage,
    ...
    fun addToFavoritesShouldAddTwo() {
        classUnderTest.addToFavorites(Activity(1))
        classUnderTest.addToFavorites(Activity(2))
    ...
    fun removeFromFavoritesShouldRemoveZero() {
        classUnderTest.removeFromFavorites(Activity(1))
    ...
    fun removeFromFavoritesShouldRemoveTwo() {
        lazyLocalStorage.resolve().project.event.myActivities.value = setOf(1, 2, 3)
        ...
        classUnderTest.removeFromFavorites(Activity(4))
        assertThat(classUnderTest.favoriteIds).hasSize(2)
        runTest {
            assertThat(classUnderTest.favoriteIdsFlow.first()).hasSize(2)
        }
    ...
    data class Activity(override val itemId: Long) : Favoriteable<Long>
}
