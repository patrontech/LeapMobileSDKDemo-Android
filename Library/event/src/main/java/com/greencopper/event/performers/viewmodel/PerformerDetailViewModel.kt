...
internal class PerformerDetailViewModel(
...
) : DetailViewModel<String>(widgetCollectionResolver, widgetResolver, myPerformersManager) {
    ...
    suspend fun getPerformer(
        ...
        hideEndTime: Boolean,
    ): Flow<DetailViewData<String>> =
        combine(
            performerRepository.getPerformerById(performerId).filterNotNull(),
            timedScheduleItemRepository.getTimedScheduleItemsForPerformer(performerId),
            myScheduleManager.favoriteIdsFlow,
            myActivitiesManager.favoriteIdsFlow,
            myPerformersManager.favoriteIdsFlow,
        ) { performer, scheduleItems, myScheduleItemIds, _, _ ->
            val itemsViewData = scheduleItems.toSortedScheduleItemsViewData(screenName, myScheduleItemIds, hideEndTime)
            performer.toDetailViewData(itemsViewData, displayableTags)
        }

    private fun Performer.toDetailViewData(
        ...
        return DetailViewData(
            ...
            widgetCollectionKey = "performer_${itemId}_detail_primary"
        )
    }
    ...
    suspend fun getPerformerDefaultName(performerId: String): String? {
        return performerRepository.getPerformerById(performerId).firstOrNull()?.name?.let {
